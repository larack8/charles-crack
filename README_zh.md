...
- 跨平台的**可执行 JAR**（`charles-keygen.jar`），在安装了 JRE 11+ 的任何系统上均可运行。
- 通过 `jpackage` 打包的**原生桌面应用 / 安装包**：
  - **macOS** → `CharlesKeygen.app` 应用包 + `.dmg` 安装镜像
  - **Windows** → `CharlesKeygen.exe` 启动器 + `.msi` 安装包
  - **Linux** → `CharlesKeygen` 二进制 + `.deb` 包（rpm 系发行版可产出 `.rpm`）
...
└── dist/                    # 最终产物（按操作系统分目录：jar / .app / .dmg / .exe / .msi / .deb …）
```
...
3. 调用 `jpackage` 在 `dist/<os>/` 下生成原生产物
...
双击原生包（`CharlesKeygen.app` / `CharlesKeygen.exe` / `./CharlesKeygen`），或在终端执行：
